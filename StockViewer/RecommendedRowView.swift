    ...
    var body: some View {
        VStack {
            HStack {
                ...
                Spacer()
                if(euro) {
                    Text(PriceFormatter.formatPrice(value: (crypto.currentPrice?["eur"]) ?? 0.00, euro: euro))
                        .foregroundColor(.gray)
                } else {
                    Text(PriceFormatter.formatPrice(value: (crypto.currentPrice?["usd"]) ?? 0.00, euro: euro))
                        .foregroundColor(.gray)
                }
            }
